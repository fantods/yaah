package register

import (
	"testing"

	"github.com/fantods/yaah/internal/provider"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestBuiltinsRegistersAllProviders(t *testing.T) {
	provider.ResetRegistry()

	Builtins()

	for _, api := range []string{"anthropic-messages", "openai-completions", "zai"} {
		got, ok := provider.Lookup(api)
		require.True(t, ok, "expected to find %q", api)
		assert.Equal(t, api, got.API)
		assert.NotNil(t, got.Stream, "%q Stream should not be nil", api)
		assert.NotNil(t, got.StreamSimple, "%q StreamSimple should not be nil", api)
	}
}

func TestBuiltinsRegistersExactlyThree(t *testing.T) {
	provider.ResetRegistry()

	Builtins()

	count := 0
	for _, api := range []string{"anthropic-messages", "openai-completions", "zai"} {
		if _, ok := provider.Lookup(api); ok {
			count++
		}
	}
	assert.Equal(t, 3, count)
}

func TestBuiltinsIdempotent(t *testing.T) {
	provider.ResetRegistry()

	Builtins()
	Builtins()

	for _, api := range []string{"anthropic-messages", "openai-completions", "zai"} {
		_, ok := provider.Lookup(api)
		require.True(t, ok, "expected to find %q after double register", api)
	}
}
